...
import { SYSTEM_INSTRUCTION, TUTOR_SYSTEM_INSTRUCTION, ENGLISH_TUTOR_SYSTEM_INSTRUCTION, TUTOR_INITIAL_MESSAGE, ENGLISH_TUTOR_INITIAL_MESSAGE, SUBJECT_INSTRUCTIONS, getSystemInstructionFromConfig, DEFAULT_SUBJECTS } from './constants';
...
export default function App() {
  ...
  const handleContextualNewChat = () => {
      ...
      if (currentView === 'study-school') {
          ...
          // Use shared DEFAULT_SUBJECTS along with customSubjects
          const allSubjects = [...DEFAULT_SUBJECTS, ...customSubjects];
          const subjName = allSubjects.find(s => s.id === activeStudySubject)?.name || activeStudySubject || "Class";
  ...
  const handleSelectSubject = (subjectId: string) => {
      ...
      const allSubjects = [...DEFAULT_SUBJECTS, ...customSubjects];
      const subjName = allSubjects.find(s => s.id === subjectId)?.name || subjectId;
  ...
  const handleStudySendMessage = async (content: string) => {
      ...
      if (!currentStudySessionId) {
          ...
          const allSubjects = [...DEFAULT_SUBJECTS, ...customSubjects];
          const subjName = allSubjects.find(s => s.id === activeStudySubject)?.name || activeStudySubject;
