...
export const LoginPage: React.FC<LoginPageProps> = ({ onLogin }) => {
  ...
  const startPasswordReset = (targetEmail: string) => {
      const code = Math.floor(100000 + Math.random() * 900000).toString();
      setGeneratedCode(code);
      setVerificationCode('');
      setPassword('');
      ...
      setSuccessMsg(`Reset code sent to ${targetEmail}`);
  };

  const getHeaderTitle = () => {
      switch (mode) {
          case 'login': return 'Welcome Back';
          case 'sign-up': return 'Get Started';
  ...
  const getHeaderSubtitle = () => {
      switch (mode) {
          case 'login': return 'Use your email or social account to continue.';
          ...
          default: return 'Complete the puzzle to verify.';
      }
  };
  ...
  if (mode === 'reset-password') {
      return (
        <div className="min-h-screen w-full flex items-center justify-center bg-gray-50 dark:bg-[#0f172a] p-4">
            <div className="w-full max-w-md bg-white dark:bg-gray-800 rounded-3xl shadow-xl border border-gray-100 dark:border-gray-700 overflow-hidden animate-in fade-in zoom-in duration-300 p-8">
                <button onClick={() => setMode('login')} className="mb-6 flex items-center gap-2 text-sm text-gray-500 hover:text-indigo-600 transition-colors">
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
                    Back to Login
                </button>

                <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Reset Password</h2>
                <p className="text-sm text-gray-500 mb-6">Enter the reset code sent to <strong>{email}</strong></p>
                ...
                <form onSubmit={handleResetSubmit} className="space-y-4">
                    {/* RESET CODE */}
                    <div>
                        <label className="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1 uppercase">Reset Code</label>
                        <div className="relative">
                            <input 
                                type="text" 
                                value={verificationCode}
                                onChange={(e) => setVerificationCode(e.target.value)}
                                className="w-full p-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none font-mono tracking-widest text-center"
                    ...
                    <button type="submit" disabled={isLoading} className="w-full py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl shadow-lg transition-all mt-4 disabled:opacity-70">
      ...
      <div className="w-full max-w-md bg-white dark:bg-gray-800 rounded-3xl shadow-xl border border-gray-100 dark:border-gray-700 overflow-hidden animate-in fade-in zoom-in duration-300 relative">
        ...
        <div className="px-8 pt-8 pb-6">
            <div className="text-center mb-8">
                ...
                <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">{getHeaderTitle()}</h1>
                <p className="text-gray-500 text-sm">{getHeaderSubtitle()}</p>
            </div>

            {error && (
                <div className="mb-5 p-3 bg-red-50 text-red-600 text-xs rounded-lg flex gap-2 items-center border border-red-100 animate-in slide-in-from-top-1">
            ...
            {successMsg && (
                <div className="mb-5 p-3 bg-green-50 text-green-600 text-xs rounded-lg flex gap-2 items-center border border-green-100 animate-in slide-in-from-top-1">
            ...
            <form onSubmit={handleStandardAuth} className="space-y-4">
                ...
                <button type="submit" disabled={isLoading} className="w-full py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl shadow-lg shadow-indigo-500/20 transition-all transform active:scale-95 flex items-center justify-center gap-2 disabled:opacity-70">
                    ...
                    {mode === 'login' ? 'Login' : mode === 'sign-up' ? 'Sign Up' : 'Send Reset Link'}
                </button>
            </form>
            ...
            <div className="mt-8 text-center text-sm text-gray-600 dark:text-gray-400">
                {mode === 'forgot' ? (
                    <button 
                        ...
                        className="font-bold text-indigo-600 hover:underline flex items-center gap-1 mx-auto"
                    >
                        Back to Login
                    </button>
                ) : (
                    <>
                        ...
                        <button 
                            onClick={() => { setMode(mode === 'login' ? 'sign-up' : 'login'); setError(''); setSuccessMsg(''); }} 
